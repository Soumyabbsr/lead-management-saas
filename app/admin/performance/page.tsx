...
export default function AdminPerformancePage() {
    ...
    const stats = useMemo(() => {
        ...
        return salesEmps.map(emp => {
            ...
            // Calls: count ActivityEvent with type 'call' or type 'note' (assuming all notes are calls for now)
            const calls = myLeads.reduce((sum, l) => sum + l.timeline.filter(e => e.type === 'call' || e.type === 'note').length, 0);

            // Reached visit stage at some point (current stage is Visit, Negotiation, Booked, or Lost but had a visit)
            ...
            return {
                id: emp.id,
                name: emp.name,
                target: emp.monthlyTarget,
                activeLeads, calls, visits, negotiation, bookings, convRate, targetPct
            };
        });
    }, [leads, employees]);
                ...
                <div style={{ background: 'linear-gradient(135deg, #2563eb, #1e40af)', borderRadius: 12, padding: 20, color: '#fff' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                            ...
                            <div style={{ fontSize: 24, fontWeight: 800 }}>{stats.length > 0 ? stats.reduce((a, b) => a.bookings > b.bookings ? a : b).name : 'No data'}</div>
                        </div>
                ...
                <div style={{ background: 'linear-gradient(135deg, #059669, #047857)', borderRadius: 12, padding: 20, color: '#fff' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                            ...
                            <div style={{ fontSize: 24, fontWeight: 800 }}>{stats.length > 0 ? stats.reduce((a, b) => a.convRate > b.convRate ? a : b).name : 'No data'}</div>
                        </div>
                    ...
                    <tbody>
                        {filteredAndSorted.map((row, i) => (
                            <tr key={row.id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background 0.1s' }}
                                ...
                                <td style={{ padding: '12px 16px', fontWeight: 700, color: '#0d1b2e' }}>{row.name}</td>
