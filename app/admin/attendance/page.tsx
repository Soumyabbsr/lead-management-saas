    ...
    return (
        <>
            <div className="att-page responsive-padding" style={{ flex: 1, padding: '24px', display: 'flex', flexDirection: 'column', gap: 20, background: '#f8fafc' }}>
                {/* Header */}
                <div className="att-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                    <div>
                ...
                <div className="att-stats" style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    {[
                        ...
                        <div key={i} style={{ flex: '1 1 160px', background: '#fff', padding: '16px 20px', borderRadius: 12, border: '1px solid #e8eef4', display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8eef4', padding: '12px 16px' }}>
                    <div style={{ position: 'relative' }}>
                        ...
                        <input
                            ...
                            style={{ width: '100%', padding: '8px 12px 8px 36px', borderRadius: 10, border: '1px solid #e2e8f0', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
                        />
                    </div>
                </div>
                            ...
                            <thead>
                                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e8eef4' }}>
                                    {['Employee', 'Status', 'First Check-in', 'Last Check-out', 'Total Time Today'].map(h => (
                                        <th key={h} style={{ padding: '14px 20px', textAlign: 'left', fontSize: 12, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {filteredEmployees.map(emp => {
                                    ...
                                    return (
                                        <tr key={emp.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                            ...
                                            <td style={{ padding: '16px 20px' }}>
                                                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 99, background: status.bg, color: status.color, fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap' }}>
                                                    {status.icon} {status.label}
                                                </div>
                                            </td>
                                            <td style={{ padding: '16px 20px', color: '#475569', fontSize: 14, fontWeight: 500, whiteSpace: 'nowrap' }}>
                                            ...
                                            <td style={{ padding: '16px 20px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                    ...
                                                    <span style={{ fontSize: 14, fontWeight: 700, color: '#0d1b2e', whiteSpace: 'nowrap' }}>
